...
morekeywords={import,package,try,raise,catch,finally,elif,read},
sensitive=false,
morecomment=[l]{\#},
morestring=[b]",
morestring=[b]',
...
I need to finalize the design of class. Python like class where fields
can be attached to the class at will or Java like class where fields
...
<print_stmt> ::= `print' [`>' "IDENT"] [<expression_list>] [`\%' "STRING"]

<read_stmt> ::= `read' [`<' "IDENT"] <expression_list> [`\%' "STRING"]
